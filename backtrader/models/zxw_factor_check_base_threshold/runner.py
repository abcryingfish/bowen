...
def run(
    *,
    ...
    if valuation_df.empty or indicator_df.empty:
        raise ValueError(
            "基本面阈值模型缺少 QMT 基本面估值数据，请先确认 "
            r"D:\database\qmt_company_data\table=factor_fundamental_valuation 已生成"
        )
    bt_df, base_filter_stats = apply_base_threshold_filter(bt_df, valuation_df, indicator_df)
    ...
    p("运行回测", 75, "Backtrader 执行 FactorCheckProfitThresholdDualAssumptionZxwStrategy")
    result = run_zxw_backtest(
        target_codes=actual_codes,
        df_multi=clip_df,
        strategy_cls=FactorCheckProfitThresholdDualAssumptionZxwStrategy,
    ...
    config_payload: dict[str, Any] = {
        ...
        "backtest_engine": "models/zxw_factor_check_base_threshold + zxw_data_pipeline 宽表",
        ...
        "initial_target_weight_by_code": init_w,
        "base_threshold_filter": base_filter_stats,
        "strategy_params": {
