...
from models.zxw_factor_check_sell_signal_profit20_step_position.daily_move_guard import DEFAULT_DAILY_MOVE_LIMIT
from models.zxw_factor_check_sell_signal_profit20_step_position.profit_threshold_strategy import (
    DEFAULT_MIN_SELL_PROFIT_MULTIPLIER,
...
def run(
    *,
    ...
    p("读取行情与信号", 25, "加载宽表（去前视）并合并前端买入/卖出因子")
    zxw = _load_zxw()
    ...
    strategy_kwargs["daily_move_limit"] = DEFAULT_DAILY_MOVE_LIMIT
    strategy_kwargs["min_sell_profit_multiplier"] = DEFAULT_MIN_SELL_PROFIT_MULTIPLIER
    strategy_kwargs["half_profit_multiplier"] = DEFAULT_HALF_PROFIT_MULTIPLIER
    strategy_kwargs["full_profit_multiplier"] = DEFAULT_FULL_PROFIT_MULTIPLIER

    p("运行回测", 75, "Backtrader 执行 FactorCheckSellSignalProfit20StepPositionZxwStrategy")
    result = run_zxw_backtest(
        target_codes=actual_codes,
        df_multi=clip_df,
        strategy_cls=FactorCheckSellSignalProfit20StepPositionZxwStrategy,
    ...
    config_payload: dict[str, Any] = {
        ...
        "backtest_engine": "models/zxw_factor_check_sell_signal_profit20_step_position + zxw_data_pipeline 宽表",
        "strategy_class": "FactorCheckSellSignalProfit20StepPositionZxwStrategy",
        "enable_future_halving_mask": False,
        ...
        "frontend_sell_signal_effective": True,
        ...
        summary_payload.update(
            {
                ...
                "因子锁定说明": "去前视（无 block_halving_future_buy）；"
                f"单日 |收盘/昨收-1|>{DEFAULT_DAILY_MOVE_LIMIT:.1%} 时该标的当天不可买卖；"
                "持仓、当前盈利超过20%、且前端卖出因子触发时，按本轮最大持股数阶梯减仓："
                "低于50%涨幅时每次卖出10%，累计最多30%；"
                "高于50%且低于100%涨幅时累计最多80%；"
                "超过100%涨幅时累计先补到80%，之后再按10%继续卖；"
