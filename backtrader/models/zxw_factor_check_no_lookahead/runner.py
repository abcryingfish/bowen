...
def run(
    *,
    ...
    """因子检验（去前视）：与 zxw_factor_check_only 相同，不算 block_halving_future_buy。"""
    ...
    p("读取行情与信号", 25, "加载宽表并合并前端买入/卖出因子（AND/OR，无未来腰斩禁买）")
    zxw = _load_zxw()
    ...
    p("运行回测", 75, "Backtrader 执行 FactorCheckZxwStrategy（去前视）")
    ...
    config_payload: dict[str, Any] = {
        ...
        "backtest_engine": "models/zxw_factor_check_no_lookahead + zxw_view_results_full 宽表",
        ...
        summary_payload.update(
            {
                ...
                "因子锁定说明": "与因子检验模型一致，但关闭 block_halving_future_buy（无年初第5日→次年第5日前视腰斩禁买）；"
