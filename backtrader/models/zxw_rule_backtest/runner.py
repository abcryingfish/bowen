...
def _run_zxw_impl(
    *,
    ...
    profit_tier_mode: str,
    progress_label: str,
    strategy_exec_label: str,
    backtest_engine: str,
    strategy_class: str,
    buy_sell_logic_tag: str,
) -> dict[str, Any]:
    def p(stage: str, value: int, message: str) -> None:
        if progress:
            progress(stage, value, message)

    p("读取行情与信号", 25, f"加载 {len(codes)} 个标的（{progress_label}，数据管线 zxw_view_results_full）")
    zxw = _load_zxw()
    ...
    if isinstance(summary_payload, dict):
        summary_payload.update(
            {
                "回测标的": actual_codes,
                "买入组合逻辑": buy_sell_logic_tag,
                "卖出组合逻辑": buy_sell_logic_tag,
                "因子锁定说明": "买卖信号由 ZXW 固定因子列与脚本逻辑决定；前端因子规则已接收但未参与合并。",
                "回测配置": config_payload,
            }
        )
...
def run(
    *,
    ...
    return _run_zxw_impl(
        ...
        frontend_sell_operator=frontend_sell_operator,
        progress=progress,
        profit_tier_mode="legacy",
        progress_label="ZXW 组合规则",
        strategy_exec_label="Backtrader 执行 ZxwRuleBacktestStrategy（legacy 分档）",
        backtest_engine="models/zxw_rule_backtest（zxw_view_results_full）",
        strategy_class="ZxwRuleBacktestStrategy",
        buy_sell_logic_tag="ZXW_RULE_FIXED_SIGNALS",
    )


def run_profit30_tier(
    *,
    codes: list[str],
    start_date: str,
    end_date: str,
    ...
    return _run_zxw_impl(
        ...
        frontend_sell_operator=frontend_sell_operator,
        progress=progress,
        profit_tier_mode="profit30",
        progress_label="ZXW 组合规则（30%盈利分档）",
        strategy_exec_label="Backtrader 执行 ZxwRuleBacktestStrategy（profit_tier_mode=profit30）",
        backtest_engine="models/zxw_rule_backtest（zxw_view_results_full，profit30 分档）",
        strategy_class="ZxwRuleBacktestStrategy",
        buy_sell_logic_tag="ZXW_RULE_PROFIT30_TIER",
    )
