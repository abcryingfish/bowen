...
def on_timer(C):
    ...
    if response_buy_stock_count >= g.params['最大买入标的数']:
        ...
        write_trade_log('已达当日最大买入标的数', response_buy_stock_count)
    ...
    for _, row in df_request.iterrows():
        if response_buy_stock_count >= g.params['最大买入标的数']:
            print('已达当日最大买入标的数', flush=True)
            write_trade_log('已达当日最大买入标的数，停止本轮处理', response_buy_stock_count)
            break
...
def get_order_id_from_obj(order_obj):
    return str(get_order_field(
        order_obj,
        ['order_id', 'm_strOrderID', 'm_nOrderID', 'm_strOrderSysID', 'm_strOrderRef'],
        ''
    ) or '').strip()
...
def infer_order_direct(order_obj, stock_code):
    trade_direct = get_order_direct(order_obj)
    if trade_direct in [23, 24]:
        return trade_direct

    write_trade_log('未完成委托方向字段未识别', {
        'stock_code': stock_code,
...
def cancel_order_if_possible(C, order_id):
    if not order_id:
        write_trade_log('撤单跳过：委托号为空')
        return

    try:
        cancel_result = cancel(order_id, g.accID, 'STOCK', C)
        ...
        print('撤单异常', order_id, e, flush=True)
        write_trade_log('撤单异常', {'order_id': order_id, 'error': str(e)})
...
def do_order(C, order):
    ...
    if order['trade_direct'] == 23:
        ...
        for i in range(max_rounds):
            ...
            if submit_volume <= 0:
                print('剩余买入金额不足一手，停止', order['stock_code'], remaining_amount, best_price, flush=True)
                write_trade_log('剩余买入金额不足一手，停止', {
                    'stock_code': order['stock_code'],
                    'remaining_amount': remaining_amount,
        ...
        if last_order_id:
            return make_process_result('submitted', '买入盘口追单结束', last_order_id)
        return make_process_result('failed', '买入盘口追单未提交委托')
...
def convert_order(order_info):
    ...
    if order['trade_direct'] == 24:
        ...
        if pos is None:
            ...
            self_cost_price, self_cost_info = get_self_position_cost(stock_code)
            if self_cost_price > 0:
                cost_price = self_cost_price
                write_trade_log('使用自维护持有成本进行卖出判断', {
                    'stock_code': stock_code,
                    'cost_price': cost_price,
                    'cost_info': self_cost_info,
                })
            else:
                # QMT 常见持仓成本字段。如果取不到，会打印持仓对象字段方便你确认。
                cost_price = getattr(pos, 'm_dOpenPrice', 0)
                write_trade_log('未找到自维护持有成本，使用QMT成本价兜底', {
                    'stock_code': stock_code,
                    'qmt_cost_price': cost_price,
