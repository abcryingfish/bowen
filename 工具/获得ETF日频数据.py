...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ETF 日频下载：xtquant 1d K 线")
    ...
    parser.add_argument("--sector-names", nargs="+", default=DEFAULT_ETF_SECTOR_NAMES, help="xtquant ETF 分类名称")
    parser.add_argument("--codes", nargs="*", default=None, help="手动指定 ETF 代码；不传则使用 ETF 分类代码池")
...
def main() -> None:
    ...
    if args.codes:
        codes = sorted({normalize_code(code) for code in args.codes if normalize_code(code)})
        sector_map = {"手动ETF": codes}
    else:
        sector_map = load_etf_sector_map([str(name).strip() for name in args.sector_names if str(name).strip()])
