#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""日频估值（get_stock_valuation）下载。

每次运行：先扫描本地 parquet；再登录并用 get_all_stocks_info 拉取上海 XSHG +
深圳 XSHE 全市场 htsc_code；与本地对比后逐只增量下载估值字段。

股票代码仅通过 API 获取。数据写入 ``D:\\database\\stock_financial_statements\\stock_valuation_data``（可通过
...
from insight_python.com.insight.query import get_all_stocks_info, get_stock_valuation

BASE_DIR = r"D:\database\stock_financial_statements\stock_valuation_data"
DEFAULT_START_DATE = "2010-01-01"
MIN_PARQUET_BYTES = 12
MERGED_FILE_NAME = "merged.parquet"
SLEEP_SEC = 0.0005

VALUATION_NUMERIC_COLUMNS: tuple[str, ...] = (
    "pe",
    "pettm",
    "pb",
    "pc",
...
VALUATION_STRING_COLUMNS: tuple[str, ...] = ("exchange",)
...
def scan_latest_downloaded_times(base_dir: str) -> dict[str, datetime]:
    ...
    try:
        ...
        print(f"发现已下载 {len(latest_time_map)} 个股票的历史估值数据。")
    except Exception as exc:
...
def normalize_stock_valuation_response(raw: pd.DataFrame) -> pd.DataFrame:
    """get_stock_valuation -> 仅保留估值字段（trading_day 映射为 time）。"""
    if raw.empty:
        return raw
    ...
    for col in VALUATION_NUMERIC_COLUMNS:
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors="coerce")

    for col in VALUATION_STRING_COLUMNS:
        if col in out.columns:
    ...
    keep_cols = [c for c in OUTPUT_COLUMNS if c in out.columns]
...
def fetch_single_stock_with_retry(
    ...
    """逐只调用 get_stock_valuation（该接口仅支持单票）。"""
    retry_count = 0
    while retry_count < max_retries:
        try:
            result = get_stock_valuation(
                htsc_code=code,
            ...
            return normalize_stock_valuation_response(result)
        except Exception as exc:
            retry_count += 1
            error_msg = str(exc)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="日频估值下载：get_all_stocks_info 全市场 + get_stock_valuation 指定字段"
    )
    parser.add_argument(
        "--listing-start",
        default="1990-01-01",
    ...
    parser.add_argument(
        ...
        help="本地尚无该票时，估值从该日起拉（默认 2010-01-01）",
    )
    parser.add_argument(
        "--end",
        default="",
        help="估值结束日，默认今天；格式 YYYY-MM-DD",
    )
    parser.add_argument(
        "--sleep-sec",
        type=float,
