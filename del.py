...
def write_trade_log(title, data=None):
    try:
        ...
        print('日志已写入:', TRADE_LOG_FILE, flush=True)
    except Exception as e:
...
def update_position_cost_on_buy(stock_code, price, volume):
    ...
    if price <= 0 or volume <= 0:
        write_trade_log('买入持有成本更新跳过：价格或数量为0', {
            'stock_code': stock_code,
            'price': price,
            'volume': volume,
        })
...
def cleanup_cleared_position_costs():
    ...
    try:
        ...
        write_trade_log('清理持有成本异常：查询持仓失败', str(e))
        return
    ...
    write_trade_log('下午1535清理已清仓持有成本完成', {
...
def log_all_trade_records(force=False):
    try:
        ...
        print('已写入交易记录日志', TRADE_LOG_FILE, flush=True)
                ...
                <TCStageLayout>
                    <control note="控件">
                        <variable note="控件">
                            ...
                            <item position="" bind="price_type_combo" value="对手价" note="价格类型" name="价格类型" type="combo" comboType="custom" list="预警价,对手价,市价单" />
...
def on_timer(C):
    ...
    for _, row in df_request.iterrows():
        if int(row.get('is_buy', 0)) == 1 and response_buy_stock_count >= g.params['最大买入标的数']:
            ...
            write_trade_log('已达当日最大买入标的数，跳过本条买入信号', {
...
def add_signal_flags(df_request):
    ...
    df_request['is_buy'] = df_request['formula'].apply(
        lambda x: 1 if str(x).strip() == '五日内六级' else 0
    )
    return df_request
...
def clear_buy_file_if_response_missing(buy_file_path, response_file_path):
    try:
        if os.path.exists(response_file_path):
            print('当天response文件已存在，不清空预警文件', response_file_path, flush=True)
            write_trade_log('启动检查：当天response文件已存在，不清空预警文件', {
        ...
        print('当天response文件不存在，已清空预警文件', buy_file_path, flush=True)
        write_trade_log('启动检查：当天response文件不存在，已清空预警文件', {
            'buy_file_path': buy_file_path,
            'response_file_path': response_file_path,
        })
        return True
...
def is_matching_submitted_order(order_obj, order_to_send):
    ...
    trade_direct = infer_order_direct(order_obj, stock_code, '新增委托方向字段未识别')
...
def get_order_direct(order_obj):
    ...
    if value in [23, '23', 48, '48'] or '买' in text or '五日内六级' in text:
        return 23

    if value in [24, '24', 49, '49'] or '卖' in text or '弱卖' in text:
        return 24
...
def build_retry_order(order_obj, remaining_volume):
    ...
    remark = get_order_field(order_obj, ['m_strRemark', 'm_strStrategyName', 'remark'], '未完成委托补单')
...
def get_order_traded_volume(order_id):
    ...
    try:
        ...
        if traded_volume is None:
            write_trade_log('委托成交量字段未识别，停止后续追单', {
                'order_id': order_id,
                'order': obj_to_dict(order_obj),
            })
            return None
        ...
        write_trade_log('查询委托成交量异常', {'order_id': order_id, 'error': str(e)})
        return None
...
def cancel_order_if_possible(C, order_id, order_obj=None):
    ...
    if order_sysid and callable(globals().get('cancel_order_stock_sysid')):
        try:
            ...
            write_trade_log('柜台合同编号撤单异常，尝试后续撤单方式', {
                'order_id': order_id,
                'order_sysid': order_sysid,
                'market': market,
                'error': str(e),
...
def check_unfinished_orders(C):
    ...
    wait_reorder_ticks = g.params.get('重新下单等待tick数', 2)
    checked_count = 0
    retry_count = 0
    ...
    for order_obj in orders:
        ...
        if trade_direct == 23 and traded_volume > 0 and order_id and order_id not in g.cost_recorded_order_ids:
            ...
            write_trade_log('从未完成买入委托回填自维护成本', {
                'order_id': order_id,
                'stock_code': stock_code,
                'price': order_price,
        ...
        if trade_direct == 23:
            ...
            if target_reached:
                ...
                continue
        write_trade_log('发现未完成委托，准备撤单后补单', {
            'order_id': order_id,
            'stock_code': stock_code,
        ...
        retry_count += 1
        write_trade_log('未完成委托补单开始', retry_order)
        do_order(C, retry_order)

    if checked_count > 0:
        write_trade_log('未完成委托检查完成', {
...
def submit_limit_order(C, order, price, volume, log_title):
    ...
    try:
        ...
        print('限价单下单异常', e, flush=True)
        write_trade_log('限价单下单异常', {'order': order_to_send, 'error': str(e)})
        return ''
...
def add_process_result_to_signal(signal_info, process_result):
    ...
    process_result = process_result or make_process_result('failed', '未返回处理状态')
...
def do_order(C, order):
    ...
    if order['trade_amount'] <= 0:
        if order.get('skip_reason'):
            ...
            write_trade_log('下单跳过：交易数量或金额为0', order)
        return make_process_result('skipped', skip_reason)

    wait_cancel_ticks = g.params.get('撤单等待tick数', 1)
    wait_reorder_ticks = g.params.get('重新下单等待tick数', 2)
    max_rounds = g.params.get('盘口最大轮数', 10)
    max_slippage_ratio = float(g.params.get('最大滑点比例', 0.03))
    ...
    if order['trade_direct'] == 23:
        if order.get('retry_unfinished_order'):
            ...
            for i in range(max_rounds):
                ...
                if not order_id:
                    write_trade_log('未获取到委托号，停止后续买入补单，避免重复下单')
                    break
                ...
                if traded_volume is None:
                    write_trade_log('买入补单已成交数量未知，停止后续追单，避免重复下单', {'order_id': order_id})
                    break
            ...
            if slippage_blocked:
                return make_process_result('skipped', '买入补单盘口价超过最大滑点', '', total_traded_volume, remaining_volume)
        ...
        for i in range(max_rounds):
            ...
            order_id = submit_limit_order(C, order, best_price, submit_volume, '买入吃卖一限价单')
            if order_id:
                last_order_id = order_id
            if not order_id:
                write_trade_log('未获取到委托号，停止后续买入追单，避免重复下单')
                break
            ...
            if traded_volume is None:
                write_trade_log('买入已成交数量未知，停止后续追单，避免重复下单', {'order_id': order_id})
                break
        ...
        if slippage_blocked:
            return make_process_result('skipped', '买入盘口价超过最大滑点', '', total_traded_volume, remaining_volume)
        return make_process_result('failed', '买入盘口追单未提交委托', '', total_traded_volume, remaining_volume)
    ...
    if order['trade_direct'] == 24:
        ...
        for i in range(max_rounds):
            ...
            order_id = submit_limit_order(C, order, best_price, submit_volume, '卖出吃买一限价单')
            if order_id:
                last_order_id = order_id
            if not order_id:
                write_trade_log('未获取到委托号，停止后续卖出追单，避免重复下单')
                break
            ...
            if traded_volume is None:
                write_trade_log('卖出已成交数量未知，停止后续追单，避免重复下单', {'order_id': order_id})
                break
        ...
        if slippage_blocked:
            return make_process_result('skipped', '卖出盘口价超过最大滑点', '', total_traded_volume, remaining_volume)
...
def get_history_cost_start_date():
    return str(g.params.get('历史成交开始日期', datetime.now().strftime('%Y%m%d')) or datetime.now().strftime('%Y%m%d'))
...
def export_official_position_costs():
    ...
    try:
        ...
        write_trade_log('导出历史成交流水成本失败：查询持仓异常', str(e))
        positions = []
...
def get_total_asset():
    try:
        ...
        write_trade_log('获取账户总资产成功', total_asset)
        return total_asset
    except Exception as e:
        print('获取账户总资产异常', e, flush=True)
        write_trade_log('获取账户总资产异常', str(e))
        return 0
...
def parse_change_percent(value):
    try:
        ...
        write_trade_log('涨跌幅解析异常', {'value': value, 'error': str(e)})
        return None
...
def convert_order(order_info):
    ...
    # 买入：五日内六级出现时，将单只股票仓位补到总资产2%
    if order['trade_direct'] == 23:
        if int(order_info.get('is_buy', 0)) != 1:
            order['trade_type'] = 1102
            order['trade_amount'] = 0
            order['trade_amount_unit'] = 'amount'
            write_trade_log('非五日内六级买入信号，跳过', order_info)
            return order
    ...
    if order['trade_direct'] == 24:
        ...
        if pos is None:
            trade_amount = 0
            print('卖出不在持仓池', stock_code, flush=True)
            ...
            if history_cost_price > 0:
                ...
                write_trade_log('历史成交流水成本为空，使用官方持仓成本兜底', {
                    'stock_code': stock_code,
                    'cost_price': cost_price,
                    'position_snapshot': get_official_position_snapshot(pos),
                })
            ...
            if can_use_volume <= 0:
                ...
                trade_amount = 0
                print('卖出失败：持仓成本为空', stock_code, flush=True)
                write_trade_log('卖出失败：持仓成本为空', {
                    'stock_code': stock_code,
                    'position': obj_to_dict(pos),
                })
            elif current_price <= 0:
                trade_amount = 0
                print('卖出失败：当前价格为空', stock_code, flush=True)
                write_trade_log('卖出失败：当前价格为空', {
                    'stock_code': stock_code,
                    'order_info': order_info,
                })
                ...
                print('价格达到持仓成本200%，清仓', stock_code, cost_price, current_price, trade_amount, flush=True)
                write_trade_log('价格达到持仓成本200%，清仓', {
                    'stock_code': stock_code,
                    'cost_price': cost_price,
                ...
                if self_cost_info.get('half_sell_done'):
                    trade_amount = 0
                    print('150%卖出已执行，未达到200%前不再卖出', stock_code, cost_price, current_price, flush=True)
                    write_trade_log('150%卖出已执行，未达到200%前不再卖出', {
                        'stock_code': stock_code,
                        'cost_price': cost_price,
                        'current_price': current_price,
                        'can_use_volume': can_use_volume,
                    ...
                    print('价格达到持仓成本150%，卖出50%', stock_code, cost_price, current_price, trade_amount, flush=True)
                    write_trade_log('价格达到持仓成本150%，卖出50%', {
                        'stock_code': stock_code,
                        'cost_price': cost_price,
                        'current_price': current_price,
                        'can_use_volume': can_use_volume,
                ...
                trade_amount = 0
                print('卖出信号出现，但价格未达到150%成本，不卖出', stock_code, cost_price, current_price, flush=True)
                write_trade_log('卖出信号出现，但价格未达到150%成本，不卖出', {
                    'stock_code': stock_code,
                    'cost_price': cost_price,
                    'current_price': current_price,
                    'can_use_volume': can_use_volume,
...
def load_tdx_signal(file_path):
    ...
    try:
        if not os.path.exists(file_path):
            print('文件不存在', file_path, flush=True)
        else:
            df = pd.read_csv(
                file_path,
                sep='\t',
    ...
    try:
        ...
        for i, p in enumerate(positions):
            position_info = {
                ...
                '总持仓': getattr(p, 'm_nVolume', None),
                '可用持仓': getattr(p, 'm_nCanUseVolume', None),
                '成本价': getattr(p, 'm_dOpenPrice', None),
                '最新价': getattr(p, 'm_dLastPrice', None),
                '市值': getattr(p, 'm_dInstrumentValue', None),
