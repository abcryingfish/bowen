...
DEFAULT_BUY_POSITION_PERCENT = 0.002
...
def get_buy_position_percent():
    ...
    if percent <= 0:
        write_trade_log('单票仓位百分比小于等�?，不买入', {'raw_value': raw_value, 'percent': percent})
        return 0
    return percent
...
def convert_order(order_info):
    ...
    if order['trade_direct'] == 23:
        ...
        if buy_amount <= 0:
            ...
            order['trade_amount'] = 0
            order['trade_amount_unit'] = 'amount'
            print('当前股票仓位已达到目标，不买�?, order['stock_code'], current_value, target_value, flush=True)
            write_trade_log('当前股票仓位已达到目标，不买�?, {
                'stock_code': order['stock_code'],
                'current_value': current_value,
                'target_percent': target_percent,
                'target_ratio': target_ratio,
        ...
        order['trade_amount_unit'] = 'amount'
        print('五日内六级买入，补足到目标仓�?, order['stock_code'], current_value, target_value, buy_amount, flush=True)
        write_trade_log('五日内六级买入，补足到目标仓�?, {
            'stock_code': order['stock_code'],
            'total_asset': total_asset,
            'target_percent': target_percent,
            'target_ratio': target_ratio,
