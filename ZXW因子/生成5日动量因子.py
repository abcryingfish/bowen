...
FACTOR_ID = "stock_momentum_5d"
FACTOR_DISPLAY_NAME = "股票5日动量"
...
def rebuild_factor(
    *,
    ...
    # 额外读取5个交易日之前的行情，避免区间起点的动量被截断。
    price_start = max(start - pd.Timedelta(days=30), pd.Timestamp("2010-01-01"))
    ...
    momentum = prices.div(prices.shift(5)) - 1.0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="重建5日动量后复权因子")
