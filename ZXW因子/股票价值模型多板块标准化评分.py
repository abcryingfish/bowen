# -*- coding: utf-8 -*-
"""基于 THS 885/886 板块生成价值模型综合评分（多板块标准化）。"""
...
def get_factor_lookback_config() -> dict[str, object]:
    return {
        ...
        "factor_lookback_days": {
            "value_model_composite_score_multi_board_normalized": 0
        },
    ...
    *,
    ...
    for snapshot_date, selected_dates in selected.items():
        ...
        if not members.empty:
            frames.append(
                pd.DataFrame({"time": selected_dates})
                .merge(members, how="cross")[_MEMBERSHIP_KEYS]
            )
    if not frames:
        return empty
    return (
...
def board_percentile_rank(
    ...
    *,
    ...
    return (
        expanded.set_index(_MEMBERSHIP_KEYS)["percentile"]
        .sort_index()
        .astype(float)
    )
...
def _align_raw_factor_frames(
    ...
    if missing:
        raise ValueError("缺少价值模型输入因子: " + "、".join(missing))
    indexes = [
...
def average_board_scores(
    ...
    *,
    ...
    return (
        average.reindex(index=normalized_dates, columns=normalized_codes).astype(float),
        count.reindex(index=normalized_dates, columns=normalized_codes).fillna(0).astype(int),
    )


def build_value_model_multi_board_normalized_score(
    raw_factor_dfs: dict[str, pd.DataFrame],
    memberships: pd.DataFrame,
    *,
    min_board_count: int = DEFAULT_MIN_BOARD_COUNT,
    ...
    weights = pd.Series(INPUT_FACTOR_WEIGHTS, dtype=float)
    valid_count = valid.sum(axis=1)
    valid_weight = valid.mul(weights, axis=1).sum(axis=1)
    weighted_sum = percentiles.fillna(0.0).mul(weights, axis=1).sum(axis=1)
    eligible = valid_count >= int(min_valid_factors)
    raw_composite = weighted_sum.div(valid_weight.where(valid_weight > 0.0)).where(eligible)
    final_percentile = _rank_board_series(
        raw_composite,
        min_board_count=min_board_count,
    )
    base_score = final_percentile * 100.0
    board_score = (
        base_score * (1.0 - MISSING_PENALTY_RATE * (1.0 - valid_weight))
    ...
    return {
        "bundle_id": BUNDLE_ID,
        "factor_dfs": {
            "value_model_composite_score_multi_board_normalized": final_score
        },
...
def build_stock_value_model_multi_board_normalized_score_bundle(
    *,
    base_dir: str | Path = DEFAULT_SIGNAL_BASE_DIR,
    snapshot_dir: str | Path = DEFAULT_BOARD_SNAPSHOT_DIR,
    start_date: str | pd.Timestamp,
    ...
    return build_value_model_multi_board_normalized_score(raw, memberships)
